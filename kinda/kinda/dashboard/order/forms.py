...
from ...order.models import Fulfillment, FulfillmentLine, Order, OrderLine, SubOrder
...
class PaymentsTemporaryForm(forms.ModelForm):
    class Meta:
        model = PaymentsTemporary
        fields = ['shipping_amount', 'shipping_payment_status', 'ref_number_shipping', 'order_payment_status', 'ref_number_order']
        labels = {
            'shipping_amount': pgettext_lazy(
                'Customer form: Given name field', 'Shipping amount'),
            'shipping_payment_status': pgettext_lazy(
                'Customer form: Given name field', 'Shipping Payment status'),
            'ref_number_shipping': pgettext_lazy(
                'Customer form: Given name field', 'Reference Number shipping'),
            'order_payment_status': pgettext_lazy(
                'Customer form: Given name field', 'Order Payment status'),
            'ref_number_order': pgettext_lazy(
                'Customer form: Given name field', 'Reference Number Order')}

class SuborderForm(forms.ModelForm):
    class Meta:
        model = SubOrder
        fields = ['status', 'payment_status', 'ref_number']
        labels = {
            'status': pgettext_lazy(
                'Customer form: Given name field', 'SubOrder Status'),
            'payment_status': pgettext_lazy(
                'Customer form: Given name field', 'Payment status to seller'),
            'ref_number': pgettext_lazy(
                'Customer form: Given name field', 'Reference Number Payment(seller)')}