...
from ...wallet.models import PaymentsTemporary
...
from .forms import (
    ...
    RefundPaymentForm, VoidPaymentForm, PaymentsTemporaryForm, SuborderForm)
from .utils import (
...
def order_details(request, order_pk):
    qs = Order.objects.select_related(
        ...
        'fulfillments__lines__order_line', 'suborders', 'temppayments')
    order = get_object_or_404(qs, pk=order_pk)
    #suborder = get_object_or_404(order.suborders)
    ...
    ctx = {
        ...
        'temppayments': order.temppayments,
        #'order_items': order.lines.filter(order=suborder.order, product_owner=suborder.order_owner),
...
@staff_member_required
@permission_required('order.manage_orders')
def update_payments(request, order_pk):
    qs = Order.objects.prefetch_related('temppayments')
    order = get_object_or_404(qs, pk=order_pk)
    payment = order.temppayments
    payment_form = PaymentsTemporaryForm(
        request.POST or None,instance=payment)
    ...
    template = 'dashboard/order/modal/payment_update.html'
    return TemplateResponse(request, template, ctx)

@staff_member_required
@permission_required('order.manage_orders')
def update_suborder(request, order_pk, suborder_pk):
    qs = Order.objects.select_related(
        'user').prefetch_related('lines__variant__product',
        'suborders', 'temppayments')
    order = get_object_or_404(qs, pk=order_pk)
    suborder = get_object_or_404(order.suborders, pk=suborder_pk)
    suborder_update_form = SuborderForm(
        request.POST or None,instance=suborder)
    if suborder_update_form.is_valid():
        suborder_update_form.save()
	
        return redirect('dashboard:order-details', order_pk=order.pk)
	
    ctx = {
        'order': order, 'suborder': suborder, 'suborder_update_form': suborder_update_form}
    return TemplateResponse(request, 'dashboard/order/modal/suborder_update.html', ctx)
    return TemplateResponse(request, 'dashboard/order/detail.html', ctx)
