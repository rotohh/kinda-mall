...
from ..core.utils.taxes import ZERO_MONEY, get_taxes_for_country, ZERO_TAXED_MONEY
...
from ..order.models import Order, OrderLine, SubOrder
...
def create_suborder(request, order):
    order_items_groups = order.lines.values('shop','order')#product_owner	 
    for order_item_group in order_items_groups:
        items = OrderLine.objects.filter(order=order_item_group['order'], shop=order_item_group['shop'])
        subtotal = (item.get_total() for item in items )
        shop = Shop.objects.get(pk=order_item_group['shop'])#user=order_item_group['product_owner']
        suborder = order.suborders.create(
            user = order.user,            
            #order_owner = order_item_group['product_owner'],
            customer_name = order.billing_address.last_name,
            customer_phone = order.billing_address.phone,
            #shop_name = shop.shop_name,
            #shop_phone = shop.phone,
            #shop_location = shop.city,
            amount = sum(subtotal, ZERO_TAXED_MONEY),
            shop = shop)
    return suborder
