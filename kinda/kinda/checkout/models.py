...
class Cart(models.Model):
    ...
    def get_shipping_price(self, taxes):
        if self.shipping_method and self.shipping_method.id == 1 and self.is_shipping_required():
            shipping_price = self.shipping_method.get_total(taxes)
        elif self.shipping_method and self.shipping_method.id != 1 and self.is_shipping_required():
            shipping_price = self.shipping_method.get_total(taxes) + self.get_shipping_price_rates()
        else :
            shipping_price = ZERO_TAXED_MONEY
        return shipping_price

    def get_shipping_price_rates(self, discounts=None, taxes=None):
        carttotals = self.get_subtotal(discounts, taxes)
        if carttotals <= TaxedMoney(Money('9.99','USD'),Money('9.99','USD')) :
            shipping_rate = (carttotals)/20
        elif carttotals >TaxedMoney(Money('9.99','USD'),Money('9.99','USD')) and carttotals <= TaxedMoney(Money('19.99','USD'),Money('19.99','USD')) :
            shipping_rate = TaxedMoney(Money('2.00','USD'),Money('2.00','USD'))
        elif carttotals >TaxedMoney(Money('19.99','USD'),Money('19.99','USD')) and carttotals <=TaxedMoney(Money('299.99','USD'),Money('299.99','USD')) :
            shipping_rate =   (carttotals)/10 
        elif carttotals >TaxedMoney(Money('299.99','USD'),Money('299.99','USD')) and carttotals <=TaxedMoney(Money('374.99','USD'),Money('374.99','USD')) :
            shipping_rate = TaxedMoney(Money('30.00','USD'),Money('30.00','USD'))
        else: 
            shipping_rate = (carttotals)/8
