...
from . import FulfillmentStatus, OrderEvents,OrderDeliveryStatus, OrderStatus, display_order_event, SuborderStatus, PaymentOutStatus
...
class Order(models.Model):
    ...
    status_order_delivery = models.CharField(
        max_length=32, default=OrderDeliveryStatus.PENDING,
...
class SubOrder(models.Model):
    order = models.ForeignKey(
        Order, related_name='suborders', editable=False, on_delete=models.CASCADE)
    user = models.ForeignKey(
        settings.AUTH_USER_MODEL, blank=True, null=True, related_name='suborders',
        on_delete=models.SET_NULL) 
    status = models.CharField(
        max_length=32, default=SuborderStatus.PENDING,
        choices=SuborderStatus.CHOICES)
    customer_name = models.CharField(blank=True, max_length=386)
    customer_phone = PossiblePhoneNumberField(blank=True, default='')
    shop = models.ForeignKey(
        Shop, related_name='shop_orders',blank=True, null=True, editable=False, on_delete=models.CASCADE)
    shop_name = models.CharField(blank=True,max_length=386)
    shop_phone = PossiblePhoneNumberField(blank=True, default='')
    shop_location = models.CharField(blank=True,max_length=386)
    ref_number = models.CharField(max_length=386, blank=True, validators=[RegexValidator('^(A-Fa-f0-9)+$', message="Not a Valid Mpesa Reference Number")])
    amount_net = MoneyField(
        currency=settings.DEFAULT_CURRENCY,
        max_digits=settings.DEFAULT_MAX_DIGITS,
        decimal_places=settings.DEFAULT_DECIMAL_PLACES,default=zero_money)
    amount_gross = MoneyField(
        currency=settings.DEFAULT_CURRENCY,
        max_digits=settings.DEFAULT_MAX_DIGITS,
        decimal_places=settings.DEFAULT_DECIMAL_PLACES,default=zero_money)
    amount = TaxedMoneyField(
        net_field='amount_net', gross_field='amount_gross')
    payment_status = models.CharField(
	    max_length=15,choices=PaymentOutStatus.CHOICES,
	    default=PaymentOutStatus.PENDING)
    #subtotal = MoneyField()
    ...
    #def save(self, *args, **kwargs):
        #if not self.pk:
            #OrderLine.objects.filter(product_owner=self.order_owner).update(suborder=self.pk)
            #SubOrder.objects.update(subtotal = self.get_shopsubtotal(sum(subtotal, ZERO_TAXED_MONEY)))
        #return super().save(*args, **kwargs)
		
    def get_shopsubtotal(self):
        orderitems = OrderLine.objects.filter(order=self.order, product_owner=self.order_owner)
        subtotal = (item.get_total() for item in orderitems)
        return sum(subtotal, ZERO_TAXED_MONEY)

    #@receiver(post_save, sender=Order)
    #def create_suborder(sender, instance, created, **kwargs):
        #if created:
            #lines =  OrderLine.objects.filter(order=instance)#sender.lines.values()
            #for line in lines:
            #lines = OrderLine.objects.filter(order=instance)#.select_related('shop')
			#orderitems = sender.objects.get(instance=instance).prefetch_related('lines')
            #SubOrder.objects.create(
                #order=instance)#shop=line.shop
                   
		
class PaymentOut(models.Model):		
    order = models.ForeignKey(
	    Order, related_name='orderpaymentout', editable=False, on_delete=models.CASCADE)
    suborder = models.ForeignKey(
	    SubOrder, related_name='suborderpaymentout', editable=False, on_delete=models.CASCADE)
...
class OrderLine(models.Model):
    ...
    suborder = models.ForeignKey(
        SubOrder, related_name='suborderlines', editable=False,blank=True, null=True, on_delete=models.CASCADE)
    ...
    shop = models.ForeignKey(
        Shop, related_name='shoporderlines',blank=True, null=True, editable=False, on_delete=models.CASCADE) 
    ...
    #def update_line(sender, instance, created, **kwargs):
        #if created:
            #suborder_items = OrderLine.objects.filter(order=instance.order, shop=instance.shop)
            #for item in suborder_items:
                #item.suborder = instance
                #item.save()
