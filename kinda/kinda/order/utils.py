...
from ..order.models import OrderLine, SubOrder
...
def add_variant_to_order(
    ...
    try:
        ...
        line.suborder = order.suborders.get().order_owner
        line.save(update_fields=['quantity', 'suborder'])
        ...
        line = order.lines.create(
            ...
            #product_owner = variant.product.user.id,
...
#def attach_suborder_to_line(order, suborder):#(line, suborder):
    #order_items = line.filter(order=suborder.order)
    #for order_item in order_items:
    #line.suborder = suborder
    #line.save()
