...
class OrderDeliveryStatus:
    ...
    CHOICES = [
        (CANCELLED, pgettext_lazy(
            'Status for a permanently canceled order',
            'Canceled')),
        (PENDING, pgettext_lazy(
            'Status for an order which has not yet been delivered',
            'pending')),
        (DELIVERED, pgettext_lazy(
            'Status for a order which has been delivered',
            'delivered'))]
...
class SuborderStatus:
    ...
    CHOICES = [
        (CANCELLED, pgettext_lazy(
            'Status for a permanently canceled suborder',
            'Canceled')),
        (PENDING, pgettext_lazy(
            'Status for a suborder which has not yet been picked',
            'pending')),
        (PICKEDFROMSELLER, pgettext_lazy(
            'Status for a suborder which has been picked from seller',
            'picked from seller'))]

class PaymentOutStatus:
    PENDING = 'pending'
    SENT = 'sent to seller'

    CHOICES = [
        (PENDING, pgettext_lazy(
            'Status for a suborder payment to seller which is pending',
            'pending')),
        (SENT, pgettext_lazy(
            'Status for a suborder payment to seller which is sent',
            'sent to seller'))]
